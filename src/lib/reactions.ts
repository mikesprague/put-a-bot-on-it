...
export const initReactions = async (msg: Message): Promise<void> => {
  ...
  const scoreMatch = scoreRegex.exec(msg.content);
  if (scoreMatch) {
    const rawScore: string = scoreMatch[1] ?? '';
    if (rawScore === 'X') {
      ...
      await (msg.channel as TextChannel).send({ content: randomGif });
    } else {
      const adjustedScore: number = Number(rawScore) - 1;
      const scoreTerms = [
        'genius',
        'magnificent',
        'impressive',
      ...
      const scoreTerm = scoreTerms[adjustedScore];
      ...
      await (msg.channel as TextChannel).send({ embeds: [gameEmbed] });
  ...
  if (msg.content.split(' ').length < 8) {
    ...
    if (messageIncludesWords(msg, lists.birdSynonyms)) {
      ...
      if (!birdleScore.test(msg.content)) {
  ...
  const rtjReaction = async (msg: Message): Promise<void> => {
    await msg.react('👉🏼');
    await msg.react('🤛🏿');
  };
