...
export default {
  data: new SlashCommandBuilder()
    .setName('earth-wind-and-fire')
    .setDescription('Random music video from YouTube API'),
  async execute(interaction: ChatInputCommandInteraction) {
    await interaction.deferReply();
    const apiUrl = `https://youtube.googleapis.com/youtube/v3/search?part=snippet&part=id&maxResults=100&order=relevance&q=earth%2C%20wind%20and%20fire&safeSearch=none&type=video&videoEmbeddable=true&key=${YOUTUBE_API_KEY}`;
