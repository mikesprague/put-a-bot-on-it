...
export const initReactions = async (msg) => {
  ...
  if (sixGuessesBirdle.test(msg)) {
    // const phewGifs = await getGiphyGifs({ searchTerm: 'phew' });
    // const randomGif =
    //   phewGifs[getRandomNum(phewGifs.length)].images.original.url;
    const phewGifs = await getTenorGifs({ searchTerm: 'phew' });
    const randomGif = phewGifs[getRandomNum(phewGifs.length)].media[0].gif.url;
    msg.channel.send({ content: randomGif });
  }
  if (failedBirdle.test(msg)) {
    // const failGifs = await getGiphyGifs({ searchTerm: 'failure' });
    // const wompWompGifs = await getGiphyGifs({ searchTerm: 'womp womp' });
