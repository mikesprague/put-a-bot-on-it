...
exports.sendContent = async (msg, content, deleteMsg = true) => {
  try {
    await msg.channel.send({ content }).then(async () => {
      if (!deleteMsg) return;
      await msg.delete();
    });
  } catch (error) {
...
exports.sendEmbed = async (msg, content, deleteMsg = true) => {
  try {
    await msg.channel.send({ embeds: [content] }).then(async () => {
      if (!deleteMsg) return;
      await msg.delete();
    });
  } catch (error) {
